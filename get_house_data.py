...
def get_house_data(url: str, session):
    ...
    if response.status_code == 200:
        ...
        house_dict['locality'] = extract_table_data(house_page, r'Neighbourhood\sor\slocality') #usually next sybling is \n
        type_of_property_raw = house_page.find('h1', {'class': 'classified__title'}).get_text().replace('\n','').replace(' ','')
        house_dict['type_of_property_cleaned'] = re.findall(r'(.*)(forsale)',type_of_property_raw)[0][0]
