...
def clean_data_set(data_frame):
    ...
    # # PROPERTY
    # print(data_frame["property_type"].unique())
    # # PRICES
    # check_values("price")
    # # NUMBER OF BEDROOMS
    ...
    # data_frame = data_frame[data_frame["price"].str.contains("month") == False]
    ...
    data_frame['Garden surface'] = data_frame['Garden surface'].replace(0, None)
    data_frame.drop(data_frame[data_frame['Garden surface'] < 5].index, inplace = True)

    # TERRACE SURFACE - replace 0 by None + remove rows with data that don't make sense
    data_frame['Terrace surface'] = data_frame['Terrace surface'].replace(0, None)
